...
| **Tustin / Bilinear（推薦）** | $s = \frac{2}{T} \cdot \frac{1 - z^{-1}}{1 + z^{-1}}$ | 左半 s 平面完整映射到 z 單位圓內，Bode 最準 |
...
**Gain Scheduling — 工業 90% 場景的真實解**：

$$
K_p(q) = K_{p_0} \cdot \frac{\det(M(q))}{\det(M_0)}, \quad K_d \text{ 保持阻尼比恆定}
$$

**物理意義**：機械臂伸展時 $M(q)$ 大 → 需高 $K_p$；摺疊時低 $K_p$；用錯會遲鈍或震盪。無人機依電池電壓動態放大 PID（Voltage Scaling）也是同理。
...
11. **工業為什麼用 Gain Scheduling 不用 RL** — 分辨學界論文 vs 工業經驗。**為什麼這是重點**：工業追求 Deterministic 穩定，Gain Scheduling 每工作點可 Bode 驗證 PM > 45°；黑箱 RL 在 OOD 狀態可能輸出極端增益，**無法通過 ISO 13849 / ISO 10218 功能安全認證**。帶出：「工業 90% 用 Gain Scheduling 依 $M(q)$ 動態調增益；不用 RL 不是技術差，是認證過不了 — 黑箱控制器在 OOD 狀態的行為無法被 Bode 或 Lyapunov 證明。」

...
| 探索機制 | OU 噪聲 | Gaussian 行為噪聲（target smoothing 屬 critic 正則化） | 策略本身的隨機性 | 最大熵 |
| 樣本效率 | 高 | 高 | 低 | 高 |
| 訓練穩定性 | 差 | 中 | 好 | 好 |
...
2. **PPO clip = 攀岩安全繩**：策略更新像攀岩，你想爬高（提高 reward），但安全繩把新舊策略的**機率比** $r_t = \pi_\theta / \pi_{\text{old}}$ 限制在 $[1-\varepsilon, 1+\varepsilon]$（通常 $[0.8, 1.2]$）區間內，超出就停止貢獻梯度。注意 clip 的是機率比，不是動作偏差或 KL 散度。TRPO 是嚴格的 KL 約束（精密安全繩），PPO 用 clip 近似（工業安全繩），更好實作但效果幾乎一樣。

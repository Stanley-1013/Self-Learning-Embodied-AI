...
- **旋轉列（後三列）與 $Z$ 無關** — 旋轉造成的像素流場只取決於像素位置本身，不受深度影響
...
**物理意義**：直接在 3D 空間做比例控制 — 平移誤差 + 旋轉誤差（axis-angle 表示）。軌跡在 Cartesian space 是直線，但精度完全依賴 $^cT_o$ 的估計品質與手眼標定精度。
...
3. **特徵點太少**：$n$ 個點給 $2n$ 行的 $L_s$，**至少需要 4 個非共面點**讓 $L_s$ 滿秩（6）

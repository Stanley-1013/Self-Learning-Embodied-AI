...
def octile(a: tuple, b: tuple) -> float:
    """Octile distance — admissible for 8-connected grid with sqrt(2) diagonal cost."""
    dx = abs(a[0] - b[0])
    dy = abs(a[1] - b[1])
...
def a_star_grid(grid: np.ndarray, start: tuple, goal: tuple) -> Optional[list]:
    ...
    8-connected with sqrt(2) diagonal cost -> uses Octile heuristic (Manhattan would over-estimate)."""

...
<details>
<summary>Deep dive: interview talking points 8-14 (advanced extensions)</summary>

8. **ACT Action Chunking + Diffusion Policy layered stack** -- the two pillars of contact-rich breakthroughs in 2022-2024. **What to say**: "ACT fixes compounding error (single-step BC errors amplify geometrically) by predicting an absolute-pose sequence for k steps + 1000 Hz low-level tracking. Diffusion Policy fixes multimodal action distributions ('go left' vs 'go right') -- MSE Transformers average both and hit the obstacle; Diffusion denoises from noise to a single mode, naturally smooth. Integration stack: VLM at 1-10 Hz emits waypoints --> ACT/Diffusion at 10-100 Hz emits low-level poses --> Cartesian impedance at 1 kHz runs $\tau = J^T[K(x_t-x)-D\dot{x}]$. The big model points the direction, the low-level impedance handles all contact-force compliance." **Why this is the key**: being able to articulate why Diffusion Policy beats MSE shows you have read Cheng Chi's paper rather than paraphrasing a blog.

9. **Residual RL assembly (sample efficiency x100)** -- the industrial-landing answer. **What to say**: "Pure RL for peg-in-hole needs millions of steps and still learns penetration; residual RL uses a classical controller $\tau_{\text{base}} = M\ddot{q}_d + C\dot{q} + G + K_p e + K_d \dot{e}$ for 90% of known dynamics and RL outputs only a small residual $\pi_\theta(s, F_{\text{ext}})$ fitting un-modeled friction --> tiny exploration space + safety fallback + sample efficiency x100. **Wide-clearance AI succeeds, sub-micron precision is still classical** because MuJoCo soft contact allows penetration --> pure RL learns to cheat --> real hardware jams on metal." **Why this is the key**: this is the line between "has shipped on real hardware" and "only runs sim".

10. **Momentum observer for collision detection without F/T** -- a core HRI skill. **What to say**: "$r(t) = K_o[M\dot{q} - \int(\tau_{\text{cmd}} + C^T\dot{q} - G + r)ds]$; the generalized momentum principle estimates external contact torque without an F/T sensor. Core technology for ISO/TS 15066 PFL certification. It is the sensing foundation of Franka / KUKA iiwa whole-body compliance." **Why this is the key**: human-robot-collaboration certification asks this.

11. **Transparent mode gravity compensation + spectral guidance/collision separation** -- kinesthetic teaching's engineering essence. **What to say**: "K->0, accurate dynamics feedforward $G(q) + \tau_{\text{fric}}$ --> release and stop, silky drag. Unexpected collision = rigid contact, very large dF/dt, high-frequency pulse; active drag = human muscles, < 2 Hz low frequency. FFT or a high-pass filter separates them to trigger e-stop or follow." **Why this is the key**: collaborative-robot vendors ask this.

12. **HRI + BCI intersection (impedance control = spinal reflex)** -- a strategic embodied-AI view. **What to say**: "BCI signals have low bandwidth and high latency; using them to command position directly is extremely dangerous. Future shared autonomy: BCI outputs high-level directional intent, impedance control takes over all physical contact, compliant avoidance, and mechanical constraints below. **Impedance control supplies the 'spinal reflex' that BCI lacks** -- this is the only path for disabled users to safely use embodied AI. Virtual fixtures (da Vinci surgery) keep K=0 inside the safe region (surgeon leads) and ramp K up as the tip approaches a vessel to deliver haptic feedback blocking dangerous cuts -- the same idea extends to BCI." **Why this is the key**: NUS embodied-AI programmes look for a 10-year architecture vision.

13. **Raibert crisp switch vs impedance continuous blend** -- the architectural essence. **What to say**: "The hybrid S matrix is a crisp black/white switch, physically forced to cut position feedback on an axis; under known geometric constraints it gives high force-control precision. Impedance M-B-K is a continuous blend, small K 'softens' the axis, more stable against unknown environments. Time-multiplexing S (S=0 before contact, S=1 after) is easy to cause command jumps and chattering at the crisp switch; modern controllers prefer the impedance soft transition." **Why this is the key**: this separates "knows hybrid by name" from "understands architecture".

14. **Contact-transition state-machine design** -- where industrial deployment most often fails. **What to say**: "The free-space-to-contact transition is the most dangerous moment for force control. Three stages: (1) on approach, pre-reduce $K_d$ from 2000 to 200, raise $D_d$ (2) force-threshold triggers the switch (e.g. 3 N), use a ramp (50-200 ms) to smoothly blend $K_d, D_d$ (3) steady-state force control with torque-saturation protection against spikes. Contact detection cannot rely on a force threshold alone; add acceleration compensation to avoid false triggers from motion inertia forces." **Why this is the key**: this issue shows up in 99% of industrial deployments.

</details>

## Further Reading

- **Hogan, "Impedance Control: An Approach to Manipulation, Parts I-III" (1985)** -- the founding trilogy of impedance control, defining the impedance/admittance causal frame, passivity-based stability, and multi-DoF extensions. Required scripture for force control
- **Raibert & Craig, "Hybrid Position/Force Control of Manipulators" (1981)** -- the original paper on selection-matrix hybrid position/force control, the theoretical basis for grinding and assembly
- **Mason, "Compliance and Force Control for Computer Controlled Manipulators" (1981)** -- the origin paper for the task-frame formalism and the natural/artificial constraint orthogonality theorem, the scripture for S-matrix physical placement
- **Khatib, "A Unified Approach for Motion and Force Control of Robot Manipulators: The Operational Space Formulation" (1987)** -- the founding paper for operational-space formulation, the $\Lambda$ inertia matrix, and task-null-space layering
- **De Luca, Albu-Schaeffer, Haddadin et al., "Collision Detection and Safe Reaction with the DLR-III Lightweight Robot Arm" (2006)** -- the classic implementation of the generalized momentum observer, the sensing basis for whole-body compliance
- **Albu-Schaeffer, Ott, Hirzinger, "A Unified Passivity-Based Control Framework for Position, Torque and Impedance Control of Flexible Joint Robots" (2007)** -- the theoretical basis for Franka / iiwa low-level impedance architectures
- **Buchli, Stulp, Theodorou, Schaal, "Variable Impedance Control -- A Reinforcement Learning Approach" (2011)** -- the representative work combining RL with impedance control, the starting point for variable impedance
- **Cheng Chi et al., "Diffusion Policy: Visuomotor Policy Learning via Action Diffusion" (RSS 2023)** -- Diffusion Policy solving multimodal action distributions, a milestone in 2022-2024 contact-rich work
- **Zhao et al., "Learning Fine-Grained Bimanual Manipulation with Low-Cost Hardware" (ACT / ALOHA, RSS 2023)** -- ACT Action Chunking solving compounding error, a new paradigm for fine bimanual manipulation
- **Lynch & Park, "Modern Robotics: Mechanics, Planning, and Control", chapter 11** -- the standard textbook on force control, covering screw theory, adjoint transforms, and hybrid control in full
- **Siciliano & Villani, "Robot Force Control" (1999)** -- a stand-alone book on force control covering impedance, admittance, hybrid, and indirect/direct force control
- **ros2_control + force_torque_sensor_broadcaster + cartesian_controllers** -- the standard ROS 2 force-control stack for quickly building impedance/admittance controllers
- **libfranka (Franka Emika)** -- production-grade Cartesian/joint impedance control API, with examples that include full adjoint-transform handling
- **MuJoCo official tutorial -- contact-rich manipulation** -- the best starting point for observing force-control behaviour in simulation, including demonstrations of the sim-to-real penetration trap
- **Xiao, Tomizuka et al., "DextAH: Dexterous Anthropomorphic Hand" series + ANYskin tactile skin** -- 2024 SOTA in dexterous manipulation, layered tactile array + RL + impedance
- **"Embodied AI Algorithm Engineer Interview Questions" chapter 6, force-control compilation** -- a consolidated set of the force-control questions most often asked in interviews, covering impedance / admittance / hybrid / variable impedance comparisons
